...
**빌드 테스트를 진행하는데, 빌드는 성공했는데 Application 구동이 안되던 상항**이었습니다.
...
원인은 **Java Application의 기본적인 상식과 관련**이 있었는데요.

<br>

> Java 언어는 코드를 컴파일하여 바이트코드를 생성하고, <br>
> 이를 JVM(Java Virtual Machine)에서 실행하는 방식을 사용합니다. <br>
> 이로 인해 Java 애플리케이션은 다른 언어로 작성된 애플리케이션에 비해 실행 속도가 느릴 수 있습니다. <br>
> 또한 Java 애플리케이션은 대부분의 경우 JVM을 포함하여 배포되므로, 애플리케이션의 크기가 상대적으로 크게 될 수 있습니다. <br>
> In. Chat GPT

<br>

관련내용은 **Java 관련 기초 상식같은 내용**이라, 

**머리로는 알고 있었지만, 한번도 체감을 해본적은 없었습니다.**

<br>

Java Application이 무거운 Application이구나 느낀 상황은 아래와 같았습니다.

<br>

#### 개노답 Java Application 3형제

파일럿 프로젝트로 Jenkins를 설치하고 CI/CD 구조를 적용하려 했을 때 당시 서버 상황은 아래와 같았는데요.
...
관습적인 root 권한에서 배포가 얼마나 편했는지,

강제적으로 jenkins, 다른 사용자 권한에서 구동하게 되니,

받게된 서버의 천박한 사용자 취급

얼마나 편리했었는지, 편리함을 핑계로 root 권한 남발 X
